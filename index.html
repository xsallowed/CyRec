...
/**
 * Recovery Time Estimator — Dynamic AI-Style MVP (single-file)
 * -----------------------------------------------------------
 * A fully functional, client-side React app that:
 *  - Runs an adaptive interview (AI-style) using a question ontology & VoI heuristic
 *  - Scores capabilities from answers and evidence flags
 *  - Runs a Monte Carlo simulation to estimate recovery time (P50/P90)
 *  - Shows live deltas, histogram, sensitivity and a prioritized action list
 *  - Saves sessions to localStorage and supports export
 *
 * No external UI libs required. Tailwind classes for style.
 * You can drop this into any React environment. Default export renders the full app.
 */
 ...
 *   topic: string,
 *   text: string,
 *   type: 'boolean'|'single-select'|'number',
 *   options?: string[],
 *   unit?: string, // for numbers
 *   weight: number, // Value-of-Information weight (0-1)
 *   evidenceHint?: string,
 *   // Mapping answer => capability adjustments or param flags
 *   onAnswer: (value, state) => Partial<DerivedState>
 * }
 */

// Capability names (scores 0..5)
const CAPS = {
...
const questionBank = [
  {
    id: "tier1_count",
    topic: CAPS.runbooks,
    text: "How many Tier-1 business services do you have?",
    type: "number",
    unit: "services",
    weight: 0.9,
    onAnswer: (v) => ({ tier1Count: clamp(Number(v)||0, 1, 50) })
  },
  {
    id: "total_tb",
    topic: CAPS.backups,
    text: "Roughly how much Tier-1 data needs restoring in a major incident?",
    type: "number",
    unit: "TB",
    weight: 0.95,
    onAnswer: (v) => ({ totalTB: clamp(Number(v)||0, 0.1, 10000) })
  },
  {
    id: "restore_concurrency",
    topic: CAPS.backups,
    text: "How many parallel restore streams can you sustain (IOPS, infra and staff)?",
    type: "number",
    unit: "streams",
    weight: 0.8,
    onAnswer: (v) => ({ concurrency: clamp(Number(v)||1, 1, 100) })
  },
  {
    id: "immutable_backups",
    topic: CAPS.backups,
    text: "Do you have immutable, offsite backups for Tier-1 systems?",
    type: "boolean",
    weight: 1.0,
    evidenceHint: "Screenshots or policy showing immutability (e.g., S3 Object Lock)",
    onAnswer: (v) => ({ immutableBackups: !!v })
  },
  {
    id: "restore_test_recency",
    topic: CAPS.backups,
    text: "When was your most recent full restore test for a Tier-1 system?",
    type: "single-select",
    options: ["<30 days", "30–90 days", ">90 days", "Never"],
    weight: 0.9,
    onAnswer: (v) => ({ restoreTestRecency: v })
  },
  {
    id: "xregion_dr",
    topic: CAPS.dr,
    text: "Is cross-region disaster recovery configured and rehearsed?",
    type: "single-select",
    options: ["No", "Configured but not rehearsed", "Rehearsed annually", "Rehearsed quarterly"],
    weight: 0.85,
    onAnswer: (v) => ({ crossRegionDR: v })
  },
  {
    id: "iam_mfa",
    topic: CAPS.identity,
    text: "Is MFA enforced for all admin accounts, with break-glass controls tested?",
    type: "boolean",
    weight: 0.7,
    onAnswer: (v) => ({ mfaAdmins: !!v })
  },
  {
    id: "edr_coverage",
    topic: CAPS.edr,
    text: "EDR/XDR coverage across endpoints and servers?",
    type: "single-select",
    options: ["<50%", "50–80%", ">80%"],
    weight: 0.7,
    onAnswer: (v) => ({ edrCoverage: v })
  },
  {
    id: "segmentation",
    topic: CAPS.netseg,
    text: "Is network segmentation in place to limit lateral movement between critical tiers?",
    type: "single-select",
    options: ["No", "Partially", "Strong (tiered, enforced)"],
    weight: 0.75,
    onAnswer: (v) => ({ segmentation: v })
  },
  {
    id: "saas_backup",
    topic: CAPS.saas,
    text: "Do you back up critical SaaS (e.g., M365/Google Workspace/Salesforce) with tested restores?",
    type: "single-select",
    options: ["No", "Backups only", "Backups + quarterly restore tests"],
    weight: 0.6,
    onAnswer: (v) => ({ saasBackup: v })
  },
  {
    id: "governance_decision_time",
    topic: CAPS.gov,
    text: "Do you have a crisis governance playbook with clear decision authority (who can pull the plug)?",
    type: "single-select",
    options: ["No", "Documented", "Documented + rehearsed"],
    weight: 0.65,
    onAnswer: (v) => ({ governanceMaturity: v })
  },
  {
    id: "vendor_slas",
    topic: CAPS.vendors,
    text: "Do your key vendors have incident SLAs (with named contacts) tested in exercises?",
    type: "single-select",
    options: ["No", "SLA only", "SLA + tested contacts"],
    weight: 0.6,
    onAnswer: (v) => ({ vendorSLA: v })
  },
  {
    id: "runbooks",
    topic: CAPS.runbooks,
    text: "Are technical recovery runbooks current (updated <90 days) and assigned owners?",
    type: "single-select",
    options: ["No", "Partial", "Yes, current and owned"],
    weight: 0.7,
    onAnswer: (v) => ({ runbooks: v })
  },
  {
    id: "staffing_sre",
    topic: CAPS.runbooks,
    text: "How many engineers can you dedicate to restore & validation concurrently?",
    type: "number",
    unit: "people",
    weight: 0.6,
    onAnswer: (v) => ({ staffCount: clamp(Number(v)||1, 1, 500) })
  },
  {
    id: "detect_mttd",
    topic: CAPS.edr,
    text: "Typical time to detect a major incident (MTTD)?",
    type: "single-select",
    options: ["<30 min", "30–120 min", ">2 hours"],
    weight: 0.6,
    onAnswer: (v) => ({ mttd: v })
  },
  {
    id: "contain_mttc",
    topic: CAPS.identity,
    text: "Typical time to contain identity compromise (MTTC) once detected?",
    type: "single-select",
    options: ["<2 hours", "2–6 hours", ">6 hours"],
    weight: 0.6,
    onAnswer: (v) => ({ mttc: v })
  },
  {
    id: "restore_throughput_tbph",
    topic: CAPS.backups,
    text: "Sustained restore throughput across infra? (TB per hour, realistic)",
    type: "number",
    unit: "TB/h",
    weight: 0.9,
    onAnswer: (v) => ({ tbph: clamp(Number(v)||0.1, 0.05, 1000) })
  },
  {
    id: "uat_cycles",
    topic: CAPS.runbooks,
    text: "How many validation/UAT cycles are required before go-live?",
    type: "number",
    unit: "cycles",
    weight: 0.5,
    onAnswer: (v) => ({ uatCycles: clamp(Number(v)||1, 1, 10) })
  },
  {
    id: "change_window",
    topic: CAPS.gov,
    text: "Can you approve emergency changes outside regular windows?",
    type: "boolean",
    weight: 0.5,
    onAnswer: (v) => ({ emergencyChange: !!v })
  },
  {
    id: "rto_target",
    topic: CAPS.runbooks,
    text: "Your current RTO target for Tier-1 services?",
    type: "number",
    unit: "hours",
    weight: 0.4,
    onAnswer: (v) => ({ rtoTarget: clamp(Number(v)||24, 1, 240) })
  },
  {
    id: "edr_signal_noise",
    topic: CAPS.edr,
    text: "Alert signal-to-noise (roughly):",
    type: "single-select",
    options: ["High noise", "Moderate", "Clean"],
    weight: 0.4,
    onAnswer: (v) => ({ edrNoise: v })
  },
  {
    id: "seg_zero_trust",
    topic: CAPS.netseg,
    text: "Any Zero Trust / strong auth between admin tiers?",
    type: "single-select",
    options: ["No", "Partial", "Strong"],
    weight: 0.4,
    onAnswer: (v) => ({ zeroTrust: v })
  },
  {
    id: "shadow_it",
    topic: CAPS.gov,
    text: "Prevalence of unmanaged shadow IT impacting recovery?",
    type: "single-select",
    options: ["High", "Some", "Low"],
    weight: 0.3,
    onAnswer: (v) => ({ shadowIT: v })
  },
  {
    id: "vendor_saas_critical",
    topic: CAPS.vendors,
    text: "Any single critical SaaS/vendor dependency without a documented fallback?",
    type: "boolean",
    weight: 0.55,
    onAnswer: (v) => ({ singleVendorRisk: !!v })
  },
  {
    id: "tabletop_recent",
    topic: CAPS.gov,
    text: "Have you run a crisis tabletop in the last 6 months?",
    type: "single-select",
    options: ["No", "Yes (single)", "Yes (multi-team)"],
    weight: 0.45,
    onAnswer: (v) => ({ tabletop: v })
  },
];
...
const DEFAULT_STATE = {
  ...
  totalTB: undefined,
  concurrency: 1,
  immutableBackups: undefined,
  restoreTestRecency: undefined,
  crossRegionDR: undefined,
  mfaAdmins: undefined,
  edrCoverage: undefined,
  segmentation: undefined,
  saasBackup: undefined,
  ...
  tbph: undefined,
  uatCycles: 1,
  emergencyChange: undefined,
  rtoTarget: 24,
  edrNoise: undefined,
  zeroTrust: undefined,
  shadowIT: undefined,
  singleVendorRisk: undefined,
  tabletop: undefined,
  ...
  // What-if overrides
  whatIf: {
    addSREs: 0, // additional staff affecting concurrency
    forceImmutability: null, // boolean | null
    forceCrossRegion: null, // string | null (matches options)
  }
};

function capabilityScores(state){
  // Base 2.5 (middle). Adjust by answers.
  const s = {
    backups: 2.5,
    dr: 2.5,
    identity: 2.5,
...
// Map capability scores -> simulation parameter multipliers
function deriveParams(state){
  const caps = capabilityScores(state);

  // helper: map 0..5 -> [low, high]
  const scale = (score, low, high) => low + (high - low) * (score / 5);

  // Detect/Decide (minutes)
  const detectModeMin = scale(caps.edr, 240, 30); // better EDR -> faster detection
  const detectMin = clamp(detectModeMin * 0.25, 5, 120);
  const detectMax = clamp(detectModeMin * 4, 60, 720);

  // Governance delay (minutes)
  const govMode = scale(caps.gov, 180, 30);
  const govMin = clamp(govMode * 0.4, 10, 90);
  const govMax = clamp(govMode * 2.5, 60, 600);

  // Contain/Eradicate (hours) via lognormal
  const containBaseMu = Math.log(scale(caps.identity, 14, 3)); // median hours
  const containSigma = scale(caps.netseg, 0.8, 0.3); // better netseg -> less variance
  ...
  const staffGate = Math.max(1, Math.floor((state.staffCount || 3) / 2)); // 2 eng per stream
  ...
  const reworkProb = immut === true ? 0.06 : immut === false ? 0.28 : 0.15; // unknown -> mid

  // Restore rework time multiplier from test recency
  let reworkMult = 1.0;
  ...
  // SaaS outage component (email/collab) in hours
  let saasHours = 4;
  ...
  // UAT / validation per cycle in hours influenced by runbooks + vendors
  const uatPer = 2.0 * (1 + (state.shadowIT === "High" ? 0.5 : state.shadowIT === "Some" ? 0.2 : 0));
  const uatCycles = Math.max(1, state.uatCycles || 1);

  const vendorDelay = state.vendorSLA === "SLA + tested contacts" ? 0.5 : state.vendorSLA === "SLA only" ? 2 : state.vendorSLA === "No" ? 6 : 3; // hours
  const singleVendorPenalty = state.singleVendorRisk ? 4 : 0; // hours

  return {
    detect: { min: detectMin, mode: detectModeMin, max: detectMax }, // minutes
    governance: { min: govMin, mode: govMode, max: govMax }, // minutes
    contain: { mu: Math.log(1) + containBaseMu, sigma: containSigma }, // hours (lognormal median around scale)
...
function runSimulation(state, runs = 2000){
  ...
  for (let i = 0; i < runs; i++){
    // Detect + Governance (minutes) -> convert to hours
    ...
    const restoreCore = p.restore.totalTB / effectiveTBph; // hours
    ...
    // Parallelism simplification: Detect+Gov → Contain → Restore (in parallel with some SaaS/vendor) → UAT → Cutover
    const tPhase1 = tDetectMin + tGovMin;
    const tPhase2 = tContain;
    const tPhase3 = Math.max(restoreCore + reworkHours, tSaaS + tVendor); // take longer of data restore vs external work
    const tPhase4 = tUAT + tCutover;

    const total = tPhase1 + tPhase2 + tPhase3 + tPhase4; // hours
    samples.push(total);
  }
...
function sensitivityAnalysis(baseState){
  const base = runSimulation(baseState, 800); // quicker for sensitivity
  const results = [];
  for (const knob of SENSITIVITY_KNOBS){
    const s = JSON.parse(JSON.stringify(baseState));
    if (knob.hasOwnProperty("on")) s[knob.key] = knob.on; 
    else if (knob.hasOwnProperty("off")) s[knob.key] = knob.off; 
    else if (knob.hasOwnProperty("delta")) s[knob.key] = Math.max(1, (s[knob.key]||0) + knob.delta);
...
function estimateVoI(q, state){
  // Heuristic: weight * (importance of the downstream param uncertainty)
  // Boost VoI if the related input not yet set.
  const key = q.id;
  const unknown = state[keyMap(key)] === undefined || state[keyMap(key)] === null;
  const base = q.weight || 0.5;
  return base * (unknown ? 1.0 : 0.2);
}
function keyMap(id){
  // In this MVP the id matches state keys for simplicity where possible
  const mapping = {
    tier1_count: "tier1Count",
    total_tb: "totalTB",
    restore_concurrency: "concurrency",
    immutable_backups: "immutableBackups",
    restore_test_recency: "restoreTestRecency",
    xregion_dr: "crossRegionDR",
    iam_mfa: "mfaAdmins",
    edr_coverage: "edrCoverage",
    segmentation: "segmentation",
    saas_backup: "saasBackup",
    governance_decision_time: "governanceMaturity",
    vendor_slas: "vendorSLA",
    runbooks: "runbooks",
    staffing_sre: "staffCount",
    detect_mttd: "mttd",
    contain_mttc: "mttc",
    restore_throughput_tbph: "tbph",
    uat_cycles: "uatCycles",
    change_window: "emergencyChange",
    rto_target: "rtoTarget",
    edr_signal_noise: "edrNoise",
    seg_zero_trust: "zeroTrust",
    shadow_it: "shadowIT",
    vendor_saas_critical: "singleVendorRisk",
    tabletop_recent: "tabletop",
  };
...
function pickNextQuestion(state, askedSet){
  ...
  // Simple branching: if immutable=false, prioritise restore test & throughput next
...
function Pill({children}){
  return <span className="px-2 py-1 rounded-full bg-slate-100 text-slate-700 text-xs">{children}</span>
}
...
function NumberStat({label, value, suffix="h", accent=""}){
  return (
  ...
  return (
    <div className="mt-2">
      ...
      <div className="flex items-center gap-2 mt-3">
        <button onClick={handleSubmit} disabled={q.type!=="boolean" && !val}
          className="px-3 py-2 bg-slate-900 text-white rounded-lg hover:bg-slate-800">Submit</button>
...
function EvidenceUpload({onFiles}){
  const ref = useRef();
  return (
    <div className="mt-2">
...
export default function RecoveryTimeEstimator(){
  ...
  // Persist
  useEffect(()=>{
    localStorage.setItem('rte_session', JSON.stringify(state));
  }, [state]);
  ...
  function runSensitivity(){
    const s = sensitivityAnalysis(state);
    setSens(s);
  }
          ...
          <Section title="Conversation">
            ...
            {currentQ ? (
              <div className="mt-4">
                ...
                <div className="mt-3">
                  <EvidenceUpload onFiles={(files)=> setState(prev => ({...prev, evidence: [...prev.evidence, ...files]}))} />
                </div>
              </div>
            ) : (
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-3 border rounded-xl">
                ...
                <div className="flex items-center gap-3">
                  <button className={`px-2 py-1 rounded ${state.whatIf.forceImmutability===true?'bg-slate-900 text-white':'border'}`} onClick={()=> setState(s=> ({...s, whatIf:{...s.whatIf, forceImmutability: true}}))}>On</button>
                  <button className={`px-2 py-1 rounded ${state.whatIf.forceImmutability===false?'bg-slate-900 text-white':'border'}`} onClick={()=> setState(s=> ({...s, whatIf:{...s.whatIf, forceImmutability: false}}))}>Off</button>
                  <button className={`px-2 py-1 rounded ${state.whatIf.forceImmutability===null?'bg-slate-900 text-white':'border'}`} onClick={()=> setState(s=> ({...s, whatIf:{...s.whatIf, forceImmutability: null}}))}>Follow answers</button>
                </div>
              </div>
              <div className="p-3 border rounded-xl">
                ...
                <div className="flex flex-wrap gap-2">
                  {["No","Configured but not rehearsed","Rehearsed annually","Rehearsed quarterly", null].map(opt => (
                    <button key={String(opt)}
                      className={`px-2 py-1 rounded ${state.whatIf.forceCrossRegion===opt?'bg-slate-900 text-white':'border'}`}
                      onClick={()=> setState(s=> ({...s, whatIf:{...s.whatIf, forceCrossRegion: opt}}))}>
              ...
              <div className="p-3 border rounded-xl">
                ...
                <input type="range" min={0} max={10} value={state.whatIf.addSREs}
                  onChange={(e)=> setState(s=> ({...s, whatIf:{...s.whatIf, addSREs: Number(e.target.value)}}))} className="w-full" />
        ...
        <div className="space-y-6">
          ...
          <Section title="Drivers & Params">
            <div className="text-sm grid grid-cols-1 gap-2">
              <div className="flex items-center justify-between"><span>Detect (mode)</span><span>{p.detect.mode.toFixed(0)} min</span></div>
              <div className="flex items-center justify-between"><span>Governance (mode)</span><span>{p.governance.mode.toFixed(0)} min</span></div>
              <div className="flex items-center justify-between"><span>Contain median</span><span>{(Math.exp(p.contain.mu)).toFixed(1)} h</span></div>
              <div className="flex items-center justify-between"><span>Total TB</span><span>{p.restore.totalTB} TB</span></div>
              <div className="flex items-center justify-between"><span>Throughput</span><span>{p.restore.tbph} TB/h</span></div>
              <div className="flex items-center justify-between"><span>Concurrency</span><span>{p.restore.concurrency} streams</span></div>
              <div className="flex items-center justify-between"><span>Rework probability</span><span>{(p.restore.reworkProb*100).toFixed(0)}%</span></div>
              <div className="flex items-center justify-between"><span>Cutover</span><span>{p.cutoverHours.toFixed(1)} h</span></div>
              <div className="flex items-center justify-between"><span>SaaS recovery</span><span>{p.saasHours.toFixed(1)} h</span></div>
              <div className="flex items-center justify-between"><span>UAT total</span><span>{p.uatHours.toFixed(1)} h</span></div>
              <div className="flex items-center justify-between"><span>Vendor/External</span><span>{p.vendorHours.toFixed(1)} h</span></div>
            </div>
          </Section>
          ...
          <Section title="Export & Session">
            <div className="flex items-center gap-2">
              <button className="px-3 py-2 rounded-lg border"
                onClick={()=> download(`rte-session-${new Date().toISOString().slice(0,19)}.json`, JSON.stringify({state, sim: {p50: sim.p50, p90: sim.p90, mean: sim.mean, sd: sim.sd}}, null, 2))}>Export JSON</button>
